...
object Graph {
  ...
  def init(prop: Properties): Unit = {
    ...
    nodeSet += Node("d", "p", 0, "执行命令")
    nodeSet += Node("x", "t", 0, "执行命令")
    nodeSet += Node("e", "t", 0, "执行命令")
    nodeSet += Node("f", "p", 0, "执行命令")
    nodeSet += Node("g", "t", 0, "执行命令")
    nodeSet += Node("h", "t", 0, "执行命令")
    nodeSet += Node("i", "t", 0, "执行命令")
    nodeSet += Node("j", "p", 0, "执行命令")
    nodeSet += Node("k", "p", 0, "执行命令")
    nodeSet += Node("l", "t", 0, "执行命令")
    ...
    // job node
    val resultSet = mysqlData.executeQuery("select name, cost, path from job_node")
    ...
    while (failedResultSet.next()) {
      ...
      if (nodeMap.contains(name)) {
        recoverSet += nodeMap.get(name).get
      }
    }
