...
默认值:5 min
TaskManager在活动资源管理器上注册超时.如果超出,活动资源管理器将释放并尝试重新请求辅助进程的资源.
如果未配置,则回退到'taskmanager.registration.timeout'.
```

---

## RestartStrategyOptions(重启策略配置)
```sh
restart-strategy
默认值:无
定义在作业失败时使用的重新启动策略
无重启策略:none,off,disable
固定延迟重启策略:fixeddelay,fixed-delay
故障率重启策略:failurerate,failure-rate
指数延迟重启策略:exponentialdelay,exponential-delay
如果CK关闭时,默认值为node,CK开启时,默认值为fixeddelay,重启次数Integer.MAX_VALUE,延迟1秒.

restart-strategy.fixed-delay.attempts
默认值:1
如果restart-strategy被设置为fixeddelay,该参数表示作业失败前重试次数.

restart-strategy.fixed-delay.delay
默认值:1 s
如果restart-strategy被设置为fixeddelay,该参数表示两次重启之间的时间间隔.

restart-strategy.failure-rate.max-failures-per-interval
默认值:1
如果restart-strategy被设置为failure-rate,该参数表示作业失败前在给定时间间隔内重新启动的最大次数.

restart-strategy.failure-rate.failure-rate-interval
默认值:1 min
如果restart-strategy被设置为failure-rate,该参数表示测试故障率的时间间隔.
...
默认值:0.1
向回退值添加随机值,避免同一时间重启多个Job.
```

---

## RestOptions(REST通信配置)
```sh
rest.bind-address
默认值:无
服务器绑定自身的地址.
...
默认值:5
REST服务器执行器处理异步请求的线程优先级.
```

---

## SecurityOptions(安全性配置)
```sh
...
默认值:无
包含用户凭据的Kerberos keytab文件的绝对路径.

security.kerberos.krb5-conf.path
默认值:无
指定krb5.conf文件的本地位置.
如果定义了,这个conf将安装在JobManager和TaskManager containers/pods上,用于Kubernetes,Yarn和Mesos.
注意:定义的KDC需要从容器内部可见.
...
默认值:false
打开SSL以便通过REST端点进行外部通信.

security.ssl.rest.authentication-enabled
默认值:false
为通过REST端点的外部通信启用相互SSL身份验证

security.ssl.keystore
默认值:无
Flink端点用于其SSL密钥和证书的Java密钥库文件.
...
默认值:无
包含Flink端点用于验证对等方证书的公共CA证书的信任库文件.
...
默认值:无
带有SSL密钥和证书的Java keystore文件,用于Flink的内部端点(rpc,数据传输,blob服务器).
...
默认值:无
带有SSL密钥和证书的Javakeystore文件,用于Flink的外部REST端点.
...
默认值:TLS_RSA_WITH_AES_128_CBC_SHA
要支持的标准SSL算法的逗号分隔列表.

security.ssl.verify-hostname
默认值:true
在SSL握手期间启用对等主机名验证的标志.
...
默认值:缓存的SSL会话对象的超时(毫秒),-1表示用系统默认值.
...
默认值:-1
刷新由关闭通道触发的"close_notify"的超时.
```

---

## TaskManagerOptions(TM配置)
```sh
...
web.ssl.enabled
默认值:true
指示是否覆盖JobManager Web UI的SSL支持的标志.

web.tmpdir
默认值:java.io.tmpdir
由WebMonitor使用的Flink Web目录.

web.upload.dir
默认值:无
上载作业jar的目录.
如果未指定动态目录,则将在JOB_MANAGER_WEB_TMPDIR_KEY指定的目录下使用.
